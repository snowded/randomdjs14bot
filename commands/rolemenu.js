    ...
    if (!target) {
        return message.reply({
            embeds: [
                new EmbedBuilder()
                    .setColor("#FF0000")
                    .setDescription("> <a:Cross:1267939449820549130> **No user found. Please mention a user or provide a valid user ID.**")
            ]
        });
    }
        ...
        for (const roleId of selectedRoles) {
            ...
            try {
                if (action === 'add') {
                    ...
                    resultMessage += `<a:Check:1267939435790598267> **Successfully __assigned__ ${role} to ${target}.**\n`;
                } else {
                    await target.roles.remove(role);
                    resultMessage += `<a:Check:1267939435790598267> **Successfully __removed__ ${role} from ${target}.**\n`;
                }
            } catch (error) {
                resultMessage += `<a:Cross:1267939449820549130> **Failed to ${action === 'add' ? 'assign' : 'remove'} ${role} ${action === 'add' ? 'to' : 'from'} ${target} | ${error.message}**\n`;
            }
        }
...
exports.help = {
    name: "rolemenu",
    description: "Create a role menu for a user",
    usage: "<@user>"
};